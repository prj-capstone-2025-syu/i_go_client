...
function RoutineDetailContent() {
  ...
  useEffect(() => {
    ...
    const fetchRoutineData = async () => {
      try {
        ...
        console.error("잔소리 목록 정보 가져오기 실패:", err);
        setError("잔소리 목록 정보를 불러오는데 실패했습니다.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error("잔소리 목록 저장 실패:", err);
      setError("잔소리 목록 저장에 실패했습니다.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const confirmDelete = async () => {
    try {
      ...
      console.error("잔소리 목록 삭제 실패:", err);
      setError("잔소리 목록 삭제에 실패했습니다.");
    } finally {
  ...
  if (loading) {
    return (
      <div className="flex flex-col w-full h-full">
        <NavBar title="잔소리 목록 상세" link="/mypage/routine"></NavBar>
  ...
  return (
    <div className="flex flex-col w-full h-full">
      ...
      <ConfirmPopup
        ...
        message={`잔소리 목록 '${title}'을 삭제하시겠습니까?`}
