...
import { Pagination, Autoplay } from "swiper/modules"; //pagination 기능 사용
...
const Home: FC = () => {
  ...
  const [nearestSchedule, setNearestSchedule] = useState<ScheduleType | null>(
    null
  );
  const [inProgressSchedule, setInProgressSchedule] =
    useState<ScheduleType | null>(null);
  const [scheduleStatusInfo, setScheduleStatusInfo] = useState<{
    text: string;
    color: string;
    fontWeight?: string;
  } | null>(null);
  const [routineName, setRoutineName] = useState<string | null>(null);
  const [currentRoutineDetails, setCurrentRoutineDetails] =
    useState<RoutineInfo | null>(null);
  ...
  const [transportTimes, setTransportTimes] = useState<{
    ...
    transit: number | null;
    walking: number | null;
    transitMessage: string | null; // 대중교통 API 제한 메시지
  }>({
    driving: null,
    transit: null,
    walking: null,
 ...
 useEffect(() => {
  ...
  // 1) window.setFCMToken 함수 등록
  (window as any).setFCMToken = (token: string) => {
    if (token) {
      console.log("✅ [Next.js] window.setFCMToken()으로 '앱 토큰' 받음:", token);
  ...
  window.addEventListener('androidFcmToken', handleFcmToken);
  ...
  return () => {
    ...
    window.removeEventListener('androidFcmToken', handleFcmToken);
  };
}, []);
  ...
  const refreshTimeAndRoutineInfo = useCallback(async () => {
    ...
    try {
      ...
      const nearestUpcoming =
        sortedSchedules.find(
          (schedule: { status: string; startTime: string | number | Date }) =>
            schedule.status !== "IN_PROGRESS" &&
            new Date(schedule.startTime) > new Date()
        ) || null;
      setNearestSchedule(nearestUpcoming);
  ...
  useEffect(() => {
    ...
    if (
      ...
      currentSchedule === inProgressSchedule
    ) {
      // 진행 중인 일정 표시
  ...
  useEffect(() => {
    ...
    if (currentSchedule) {
      ...
      const hasStartCoords =
        currentSchedule.startX != null && currentSchedule.startY != null;
      const hasDestCoords =
        currentSchedule.destinationX != null &&
        currentSchedule.destinationY != null;
      ...
      if (!hasStartCoords && !hasDestCoords) {
        console.log(
          "🚦 [DEBUG] 출발지 및 목적지 좌표 정보 부족으로 이동시간 계산 불가"
        );
        setIsTransportLoading(false);
        return;
      }
        ...
        const calculateTimes = async () => {
          try {
            ...
            if (
              ...
              console.error("🚦 [ERROR] 유효하지 않은 좌표값:", {
                startX,
                startY,
                endX,
                endY,
            ...
            console.log("🚦 [DEBUG] 최종 이동시간 계산 좌표:", {
              startX,
              startY,
              endX,
              endY,
            ...
            const times = await calculateAllTransportTimes(
              startX,
              startY,
              endX,
              endY
            ...
            console.log(
              `🚦 [DEBUG] calculateAllTransportTimes() 완료 (${(
                endTime - startTime
              ).toFixed(2)}ms)`,
              times
            );
            ...
            setTransportTimes({
              ...
              transitMessage: null, // 초기화
            });

            // 4. 대중교통 API 호출 제한 메시지 처리
            if (times.transit === null) {
              setTransportTimes((prev) => ({
                ...prev,
                transitMessage:
                  "대중교통 API 호출 제한으로 인해 시간이 표시되지 않습니다.",
              }));
            }

            // 5. 데이터 로딩 완료 후 애니메이션 효과 표시
            setTimeout(() => {
              setIsTransportLoading(false);
  ...
  const handleTmapNavigation = () => {
    ...
    if (hasStartCoords && hasDestCoords) {
      if (isMobile) {
        ...
        // PC: Naver 지도로 길찾기
        const startName = encodeURIComponent(
          currentSchedule.startLocation || "출발지"
        );
        const destName = encodeURIComponent(
          currentSchedule.location || "도착지"
        );
  ...
  return (
    <div className="flex flex-col w-full h-full">
      ...
      {!isTokenChecked || !isAuthenticated ? null : ( // 토큰 확인 중이거나 인증되지 않은 경우 메인 콘텐츠 표시하지 않음
        <>
          <div className="w-full max-h-full overflow-y-auto">
            <div className="flex flex-col items-center justify-start p-[20px] w-full h-auto">
              ...
              >
                ...
                />
                <div className="absolute flex !right-[11px] w-[25px] !top-[50%] !translate-y-[-50%]">
                  <button
                    className="w-full p-[4px] cursor-pointer"
                    type="submit"
                  >
                    <svg
                ...
                {scheduleStatusInfo && scheduleDataReady && (
                  <p>
                    <span
                      ...
                      style={{
                        color: scheduleStatusInfo.color,
                        fontWeight: scheduleStatusInfo.fontWeight || "600",
                      }}
                    >
              ...
              >
                {/* 로딩 중 */}
                {isLoading && (
                  <div className="w-full flex justify-center items-center gap-x-[15px]">
                    <p className="text-[#383838] text-center py-2">로딩 중</p>
                    <span>
                      <img
                        className="mr-3 -ml-1 w-[18px] animate-spin text-white"
                ...
                {scheduleDataReady && currentSchedule && (
                  <div className="flex flex-col w-full">
                    ...
                    {currentSchedule.location && (
                      <p className="text-[#383838] text-[15px] font-[400] tracking-[-0.1px] leading-[160%] line-clamp-1 text-left">
                    ...
                    {currentSchedule.supplies && (
                      <p className="text-[#383838] text-[15px] font-[400] tracking-[-0.1px] leading-[160%] text-left">
                    ...
                    <div className="my-[16px] w-full h-[1px] bg-[#dfdfdf]"></div>
                      ...
                      <div className="flex items-center gap-x-[1px]">
                        <span className="text-[#01274f] text-[15px] font-[600] tracking-[-0.8px] leading-[102%] line-clamp-1">
                          {currentRoutineDetails
                            ? `${currentRoutineDetails.name} (${
                                currentRoutineDetails.totalDurationMinutes ||
                    ...
                    <Swiper
                      ...
                      className="!w-full !flex !11111111111flex-col gap-[9px]"
                    >
                      {currentRoutineDetails &&
                      currentRoutineDetails.items &&
                      currentRoutineDetails.items.length > 0 &&
                      currentSchedule &&
                        ...
                        (() => {
                          ...
                          return currentRoutineDetails.items.map((item) => {
                            ...
                            if (currentTime >= itemEndTime) {
                              ...
                              itemStatus = "진행 중";
                              itemIcon = "⏭️";
                              itemBgColor = "bg-[#2DB400] animate-pulse";
                            }

                            return (
                              <SwiperSlide
                                key={item.id}
                                className={` !h-[auto] px-[10px] py-[5px] ${itemBgColor} rounded-[6px] !flex flex-col items-center justify-between transition-colors duration-300 ease-in-out gap-y-[6px]`}
                              >
                                <p className="text-[#fff] text-[16px] font-[600] tracking-[-0.5px] leading-[102%] line-clamp-1">
                                  {item.name}
                                </p>
                                <div className="flex items-center gap-x-[4px]">
                                  <span className="text-[#fff] text-[14px] font-[600] tracking-[-0.8px] leading-[102%] line-clamp-1 ml-1">
                                    {item.durationMinutes}분
                                  </span>
                                  <span className="text-[14px] leading-[120%]">
                                    {itemIcon}
                                  </span>
                                  <span className="text-[#fff] text-[14px] font-[600] tracking-[-0.8px] leading-[102%] line-clamp-1">
                                    {itemStatus}
                                  </span>
                                </div>
                              </SwiperSlide>
                    ...
                    {isRemoteEvent ? (
                      ...
                      (currentSchedule &&
                        (currentSchedule.startX == null ||
                          currentSchedule.startY == null ||
                          currentSchedule.destinationX == null ||
                          currentSchedule.destinationY == null)) ? (
                      ...
                      <>
                        <div className="grid grid-cols-3">
                          <div className="h-auto flex gap-x-[5px] items-center justify-center">
                            ...
                            <p
                              className={`group-hover:!text-[#fff] text-[#01274F] text-[14px] font-[500] tracking-[-0.8px] leading-[110%] transition-opacity duration-500 ${
                                showTransportTimes ? "opacity-100" : "opacity-0"
                              }`}
                            >
                              {!isTransportLoading &&
                              transportTimes.driving !== null
                                ? formatTransportTime(transportTimes.driving)
                                : ""}
                            </p>
                            {isTransportLoading && (
                          ...
                          <div className="h-auto flex gap-x-[5px] items-center justify-center">
                            ...
                            {/* 대중교통 시간 또는 API 제한 메시지 표시 */}
                            {!isTransportLoading && (
                              <>
                                {transportTimes.transit !== null ? (
                                  <p
                                    className={`group-hover:!text-[#fff] text-[#01274F] text-[14px] font-[500] tracking-[-0.8px] leading-[110%] transition-opacity duration-500 ${
                                      showTransportTimes
                                        ? "opacity-100"
                                  ...
                                  >
                                    {formatTransportTime(
                                      transportTimes.transit
                                    )}
                                  </p>
                                ) : transportTimes.transitMessage ? (
                                  <div className="relative inline-block">
                                    <p
                                      className={`group-hover:!text-[#fff] text-[#FF6A00] text-[14px] font-[500] tracking-[-0.8px] leading-[110%] transition-opacity duration-500 ${
                                        showTransportTimes
                                          ? "opacity-100"
                                          : "opacity-0"
                                      } cursor-help underline decoration-dotted`}
                                      title={transportTimes.transitMessage}
                                    >
                                      제한됨
                                    </p>
                                    <div className="absolute z-10 bottom-full left-1/2 transform -translate-x-1/2 w-[200px] bg-white shadow-lg rounded-lg p-2 text-xs text-gray-700 hidden group-hover:block">
                                      {transportTimes.transitMessage ||
                                        "API 호출 제한으로 정보를 가져올 수 없습니다"}
                                    </div>
                                  </div>
                                ) : (
                                  <p
                          ...
                          <div className="h-auto flex gap-x-[5px] items-center justify-center">
                            ...
                            >
                              {!isTransportLoading &&
                              transportTimes.walking !== null
                                ? formatTransportTime(transportTimes.walking)
                                : ""}
                            </p>
                            {isTransportLoading && (
                              <span className="inline-block w-5 h-5 border-2 border-t-transparent border-[#01274F] rounded-full animate-spin"></span>
                          ...
                          >
                            ...
                            <defs>
                              ...
                              <image
                                ...
                                height="1440"
                                preserveAspectRatio="none"
                                xlinkHref="data:image/png;base64,"
                              />
                            </defs>
                          </svg>
